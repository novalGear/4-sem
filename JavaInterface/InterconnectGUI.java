...
public class InterconnectGUI extends JFrame implements ActionListener {
    ...
    private void handleButtonRunAction() {
        ...
        // 1. Считываем выбор топологии
        String selectedTopology = radioButtonMesh.isSelected() ? "MESH" : "BUTTERFLY";
        log("Topology: " + selectedTopology);

        // 2. Считываем выбор роутинга
        String selectedRouting = radioButtonOblivious.isSelected() ? "OBLIVIOUS" : "ADAPTIVE";
        log("Routing: " + selectedRouting);

        // 3. Считываем размеры
        try {
            int width = Integer.parseInt(textFieldWidth.getText().trim());
            int height = Integer.parseInt(textFieldHeight.getText().trim());
            int nodes = Integer.parseInt(textFieldNodes.getText().trim());

            log("Dimensions: " + width + " x " + height);
            log("Nodes count: " + nodes);
            log("SUCCESS: Parameters validated. Ready to call C++ backend.");

        } catch (NumberFormatException exception) {
            log("ERROR: Please enter valid integer numbers for dimensions!");
